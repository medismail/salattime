		...
		<div id="app-content"><div id="app-content-wrapper" class="salattime-content">
			<section v-if="view === 'overview'" class="salattime-page">
				...
				<div class="salattime-grid">
					...
					<section class="salattime-card"><h2>{{ t('Moon and Sun Information') }}</h2><dl class="salattime-facts"><dt>{{ t('Moon Phase') }}</dt><dd>{{ state.MoonPhase }}</dd><dt>{{ t("Moon's illuminated fraction") }}</dt><dd>{{ state.IlluminatedFraction }}%</dd><dt>{{ t('New moon') }}</dt><dd>{{ state.NewMoon }}</dd><dt>{{ t('Next new moon') }}</dt><dd>{{ state.NextNewMoon }}</dd><dt>{{ t('Sun Position') }} ({{ t('Azimuth') }}/{{ t('Altitude') }})</dt><dd></dd><dt></dt><dd>{{ state.SunAzimuth }}° / {{ state.SunAltitude }}°</dd><dt>{{ t('Moon Position') }} ({{ t('Azimuth') }}/{{ t('Altitude') }})</dt><dd></dd><dt></dt><dd>{{ state.MoonAzimuth }}° / {{ state.MoonAltitude }}°</dd></dl></section>
				</div>
			</section>
...
export default {
	...
	computed: {
        ...
        methods() { return [{ value: 'MWL', label: this.t('Muslim World League') }, { value: 'MAKKAH', label: this.t('Umm Al-Qura University, Makkah') }, { value: 'KARACHI', label: this.t('University of Islamic Sciences, Karachi') }, { value: 'ISNA', label: this.t('Islamic Society of North America (ISNA)') }, { value: 'JAFARI', label: this.t('Shia Ithna-Ashari, Leva Institute, Qum') }, { value: 'GULF', label: this.t('Gulf Region') }, { value: 'MOONSIGHTING', label: this.t('Moonsighting Committee Worldwide (Moonsighting.com)') }, { value: 'TURKEY', label: this.t('Diyanet İşleri Başkanlığı, Turkey') }, { value: 'TEHRAN', label: this.t('Institute of Geophysics, University of Tehran') }, { value: 'EGYPT', label: this.t('Egyptian General Authority of Survey') }, { value: 'QATAR', label: this.t('Qatar') }, { value: 'KUWAIT', label: this.t('Kuwait') }, { value: 'TUNISIA', label: this.t('Tunisia') }, { value: 'INDONESIA', label: this.t('Kementerian Agama Republik Indonesia') }, { value: 'MOROCCO', label: this.t('Morocco') }, { value: 'JAKIM', label: this.t('Jabatan Kemajuan Islam Malaysia (JAKIM)') }, { value: 'JORDAN', label: this.t('Ministry of Awqaf, Islamic Affairs and Holy Places, Jordan') }, { value: 'ALGERIA', label: this.t('Algeria') }, { value: 'RUSSIA', label: this.t('Spiritual Administration of Muslims of Russia') }, { value: 'FRANCE', label: this.t('Union of Islamic Organizations of France (UOIF)') }, { value: 'PORTUGAL', label: this.t('Comunidade Islamica de Lisboa') }, { value: 'SINGAPORE', label: this.t('Majlis Ugama Islam Singapura, Singapore') }]},
