...
class DecRaDist extends DecRa {
	...
	public function __construct($d, $r, $dist) {
...
class AzAlt {
	...
	public function __construct($az, $alt) {
		$this->azimuth = $az;
		$this->altitude = $alt;
	}
}
...
class AzAltDist extends AzAlt {
	...
	public function __construct($az, $alt, $dist) {
...
class SunCalc {
	...
	public function __construct($date, $lat, $lng) {
		$this->date = $date;
		$this->lat = $lat;
		$this->lng = $lng;
	}
	...
	public function getMoonTimes($inUTC = false) {
		$t = clone $this->date;
		if ($inUTC) {
		...
		if ($set != 0) {
			$result['moonset'] = hoursLater($t, $set);
		}
